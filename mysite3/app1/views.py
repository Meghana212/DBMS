...
def index(request):
    if request.method=='GET':
        ...
        depart = str(depart)
        depart = depart + '%'
        res_count=cursor.execute("SELECT Class,Fare,Departure from fare f,flight_schedule fs,Route r where r.source_airport=%s and r.destination_airport = %s and r.R_ID=f.Route and f.Route = fs.R_ID and fs.Departure like %s",[s,d,depart])
        if res_count>0:
            avf = cursor.fetchall()
            connection.close()
...
def forms(request):
    ...
    if request.method=='POST':
        ...
        p1 = Passenger(firstname=fname,middlename=mname,lastname=lname,email=em,nationality=nationality,age=age,phone_no=ph_no)
